...
//
// В правилах с пятнадцатого дня стоит карта переходов: где задача сейчас
// и куда из этого места можно. Раньше там было расплывчатое «не перепрыгивай
// стадии» — совет, который модель нарушает ровно так же, как человек
// переходит дорогу на красный.
func (t *Task) Block() string {
	...
	b.WriteString(t.transitionRules())
	return b.String()
}
