...
- "chat" — краткосрочная память этого разговора. Не переживёт его и не должна пережить. Сюда: какой из вариантов сейчас обсуждаем, о чём пользователь попросил напомнить позже, на чём прервались, — то, что завтра в новом разговоре будет только мешать.
...
- слой выбирается по времени жизни, а не по важности. Спроси себя: это пригодится в следующем разговоре об этой же задаче? Если нет — "chat". Если да, но кончится вместе с задачей — "task". Если переживёт и задачу — "user";
