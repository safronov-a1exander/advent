...
//	    платежи.md
//
// Формат правила — markdown с необязательным frontmatter:
//
//	---
//	forbid: [python, django, fastapi]
//	unless: [нельзя, не подходит, не будем]
//	---
//
//	Бэкенд только на Go. Python и Node.js предлагать нельзя ни в каком виде.
//
// Тело файла — сама формулировка, и она обязательна; frontmatter — нет.
// Markdown, а не YAML, ровно потому, что формулировка здесь главное:
// её читает модель в промпте и пользователь в объяснении отказа. В YAML
// она была бы строкой в кавычках, которую надо экранировать из-за каждого
// двоеточия, — а править её будут чаще всего остального.
//
...
func (s *Set) Summary() string {
	...
	return fmt.Sprintf("%s · правил %d (с быстрым фильтром %d)", name, len(s.Invariants), filtered)
}
